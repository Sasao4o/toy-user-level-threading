...
static void schedule(void* _) {
    ...
    while (runq_front != NULL) {
        ...
        if (next_thread->state == STATE_RUNNABLE) {
            runq_push_back(next_thread);  // only push back if still runnable
        } else {
...
static struct thread* thread_new(threadfn_t fn, void* arg) {
    ...
    sp = (uint8_t*)((uintptr_t)sp & ~0xF); // align to 16 bytes 
    
    sp -= 8;
    *(uint64_t*)sp = (uint64_t)arg;
    
    sp -= 8;
    *(uint64_t*)sp = (uint64_t)fn;
...
bool thread_yield(void) {
    ...
    if (runq_front == NULL) {
        ...
        struct thread *prev_running = running;
        ctxswitch(&prev_running->ctx, &scheduler->ctx);
        return true;
    }
 }
